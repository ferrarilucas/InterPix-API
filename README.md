...
- **`POST /pix/v2/rec` foi confirmado em produção**: o Inter aceita o corpo enviado (`vinculo`, `calendario`, `valor`, `politicaRetentativa`, `loc`), devolve `201` com `idRec` no formato esperado (`RR...`) e `status: "CRIADA"`. A resposta de criação **nunca** traz `dadosQR` — o Pix Copia-e-Cola só sai do `GET /pix/v2/rec/{idRec}` de acompanhamento, não é um caminho alternativo raro.
- **Esse `GET` de acompanhamento pode retornar 404 nos primeiros instantes após a criação** — lag de propagação do lado do Inter, observado em produção (404 a 111ms da criação; tentativas subsequentes, minutos depois, tiveram sucesso). `getRecurrence` tenta de novo automaticamente após um 404 (400ms, depois 1200ms; três tentativas no total) antes de desistir. Ainda não sabemos o limite superior real desse atraso — se ele ultrapassar ~1.6s em algum caso, a criação da assinatura falha com `UPSTREAM_ERROR` mesmo a recorrência tendo sido criada com sucesso no Inter (fica órfã lá, sem `cancelRecurrence`).
- **`loc` (identificador de location) é obrigatório em `POST /rec` para o Pix Copia-e-Cola existir — não está na documentação pública do Inter, só na spec do Bacen.** Sem ele, o Inter aceita a recorrência normalmente (`201`, `CRIADA`) mas nunca compõe `dadosQR`, porque não há location associada para gerar o QR. Isso foi observado em produção: três assinaturas criadas em sequência, todas sem erro, todas sem Pix Copia-e-Cola. A correção cria a location primeiro (`POST /pix/v2/locrec`, sem corpo) e referencia o `id` retornado no campo `loc` do `POST /rec`. Isso exige o escopo OAuth `payloadlocationrec.write` (leitura: `payloadlocationrec.read`), que **não fazia parte do pedido de token até este ponto** — se a aplicação cadastrada no portal do Inter não tiver esse escopo habilitado, o pedido de token inteiro é recusado, derrubando toda a integração, não só a criação de assinatura. Confirme no portal do Inter que o escopo está habilitado antes de subir esta mudança.
- **`cancelRecurrence` detecta "recorrência já terminal" consultando o status real no Inter, não só pelo texto do erro.** O Inter varia a frase do erro `400` para a mesma situação — já observamos duas: `"Recorrencia ja esta CANCELADA"` e `"A recorrência a ser alterada possui status diferente do necessário para a alteração"` (a segunda corresponde à violação documentada na spec do Bacen para `PATCH /rec/{idRec}`: "A recorrência encontra-se expirada, cancelada ou rejeitada"). Casar por palavra-chave é frágil a variações futuras de texto, então em qualquer `400` não reconhecido pelas palavras-chave, o código faz um `GET /rec/{idRec}` de confirmação: se o status real já for `CANCELADA`, `REJEITADA` ou `EXPIRADA`, trata como sucesso (a recorrência já não vai gerar mais cobranças, que é o efeito desejado); senão, propaga o erro normalmente.
- **O payload de `createCharge` (`PUT /pix/v2/cobr/{txid}`) ainda não foi verificado ponta a ponta contra o sandbox do Inter.** O corpo inteiro — incluindo o formato do objeto `recebedor` e os valores aceitos de `tipoConta` — foi montado a partir da documentação pública do Inter. O mesmo vale para `vinculo.contrato` (enviado como `planCode` em `POST /rec`, que agora sabemos ser aceito — mas não confirmamos que o valor precisa ser distinto por cobrança ou pode repetir). Também não confirmados: os nomes dos campos `endToEndId`, `horario` (mapeado para `paidAt`) e `motivoRejeicao` (mapeado para `failureReason`) na resposta de uma cobrança liquidada ou rejeitada — são inferências, não confirmações da doc real. Se algum desses nomes estiver errado, uma cobrança paga de verdade mapeia para status `UNKNOWN` em vez de `PAID`/`FAILED`, e a assinatura correspondente nunca sai de `PENDING_AUTH`/fica presa sem ativar — esse é o modo de falha a observar em sandbox antes de ir para produção.
