...
- **`POST /pix/v2/rec` foi confirmado em produção**: o Inter aceita o corpo enviado (`vinculo`, `calendario`, `valor`, `politicaRetentativa`), devolve `201` com `idRec` no formato esperado (`RR...`) e `status: "CRIADA"`. Uma divergência real da documentação: a resposta de criação **nunca** traz `dadosQR` — o código sempre precisa do `GET /pix/v2/rec/{idRec}` de acompanhamento para obter o Pix Copia-e-Cola, não é um caminho alternativo raro.
