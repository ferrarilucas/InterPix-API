...
export async function createDueCharge(data: CreateRecurringChargeRequest): Promise<RecurringPixCharge> {
  try {
...
export async function getDueCharge(txid: string): Promise<RecurringPixCharge> {
  try {
...
export async function authorizeDueCharge(txid: string): Promise<AuthorizeRecurringChargeResponse> {
  const data = {
    "txid": txid
  }
  try {
